...
class RV64A : public RevExt {
  ...
  std::vector<RevInstEntry> RV64ATable = {
    {RevInstEntryBuilder<Rev64AInstDefaults>().SetMnemonic("lr.d %rd, (%rs1)"          ).SetFunct7(0b00010                        ).Setrs2Class(RevRegClass::RegUNKNOWN).SetImplFunc(&lrd ).InstEntry},
    {RevInstEntryBuilder<Rev64AInstDefaults>().SetMnemonic("sc.d %rd, %rs2, (%rs1)"    ).SetFunct7(0b00011                                                             ).SetImplFunc(&scd ).InstEntry},
